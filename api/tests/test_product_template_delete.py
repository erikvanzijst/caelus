...
def test_delete_product_flow(client):
    # create product
    resp = client.post("/api/products", json={"name": "testprod", "description": "desc"})
...
def test_delete_template_flow(client):
    # create product
    prod_resp = client.post("/api/products", json={"name": "tmplprod", "description": "desc"})
    assert prod_resp.status_code == 201
    prod_id = prod_resp.json()["id"]
    # create template
    tmpl_resp = client.post(
        f"/api/products/{prod_id}/templates",
        json={"chart_ref": "registry.home/nextcloud/", "chart_version": "1.0.0"},
    )
    assert tmpl_resp.status_code == 201
    tmpl_id = tmpl_resp.json()["id"]
    # delete template
    del_resp = client.delete(f"/api/products/{prod_id}/templates/{tmpl_id}")
    assert del_resp.status_code == 204
    # get should 404
    get_resp = client.get(f"/api/products/{prod_id}/templates/{tmpl_id}")
    assert get_resp.status_code == 404
    # list templates should not include
    list_resp = client.get(f"/api/products/{prod_id}/templates")
    assert list_resp.status_code == 200
