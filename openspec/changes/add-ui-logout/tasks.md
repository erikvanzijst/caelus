...
- [ ] 2.2 Set `entryPoints` to `["web", "websecure"]` to match the Caelus
      ingress

## 3. Keycloak — client configuration (manual)

- [ ] 3.1 In Keycloak admin console, open the `caelus-dev` client under the
      `master` realm
- [ ] 3.2 Add `https://app.deprutser.be/` and `https://dev.deprutser.be/` to
      **Valid Post Logout Redirect URIs**

## 4. Frontend — useAuthEmail clearHeaders

- [ ] 4.1 Export a `clearStoredAuthHeaders()` function from
      `ui/src/state/useAuthEmail.ts` that removes the `caelus.auth.headers`
      key from localStorage

## 5. Frontend — logout dropdown menu

- [ ] 5.1 In `ui/src/components/AppShell.tsx`, replace the static `Chip` with a
      clickable element (MUI `Chip` with `onClick` or a `Button`) that opens an
      MUI `Menu`
- [ ] 5.2 Add a `Logout` `MenuItem` to the menu
- [ ] 5.3 Implement the logout handler:
...
- [ ] 5.4 Add necessary MUI imports (`Menu`, `MenuItem`, and `useState` for
      anchor element)

## 6. Deploy and verify

- [ ] 6.1 Apply Terraform changes (`tf/app/`) in the dev workspace
- [ ] 6.2 Verify the IngressRoute is created and `/oauth2/sign_out` on
      `dev.deprutser.be` routes to oauth2-proxy (curl or browser)
- [ ] 6.3 Build and deploy the updated UI
- [ ] 6.4 End-to-end test: log in, click Logout, confirm landing on Keycloak
      login page, confirm re-login requires credentials (Keycloak session was
      terminated)
- [ ] 6.5 Test local dev: start UI with `npm run dev`, enter email in dialog,
