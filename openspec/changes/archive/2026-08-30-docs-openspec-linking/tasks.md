## 1. `AGENTS.md` Architecture Notes (design.md § D7 — highest leverage)

- [x] 1.1 Audit the 80-line Architecture Notes section into design.md § D6's
...
- [x] 1.2 Rewrite Architecture Notes as terse entries plus links: builds
      ...
      pointers with links into `openspec/`. Verify the two-CLIs distinction
      (`caelus` vs `freepod`) survives — it is bucket 2 and spans capabilities.
- [x] 1.3 Add to § Documentation Layering the rule that renaming or merging a
      capability directory updates the prose links pointing at it in the same
      change (design.md § D5). Verify the section still reads coherently.

## 2. `api/README.md` (~1,280 of 1,501 lines in scope)

- [x] 2.1 Audit all 28 sections into the three buckets; confirm § Codebase Map,
...
- [x] 2.2 Convert § Authentication (285 lines) → `auth-header-integration`,
...
- [x] 2.3 Convert § Builds (171 lines) → `build-api`, `build-artifact-upload`,
      `build-data-model`, `build-execution`, `build-worker`. Verify links
      resolve.
- [x] 2.4 Convert § Product Catalog (170 lines) → `product-catalog-format`,
      `catalog-reconciliation`, `catalog-cli`, `curated-product-governance`,
      `product-visibility`. Verify links resolve.
- [x] 2.5 Convert § Account SSH Keys (130 lines) → `ssh-key-api`,
...
- [x] 2.6 Convert § Per-Deployment Relational Storage (108) and § Reading a
...
- [x] 2.7 Convert § Core Data Model (108) → the data-model capabilities
...
- [x] 2.8 Convert § Deployment Vars (107) → `deployment-vars-api`,
      `deployment-vars-data-model`, `deployment-vars-schema-routing`,
      `deployment-vars-reconciliation`. Verify links resolve.
- [x] 2.9 Convert § API and CLI Parity (73), § Deployment Lifecycle and State
      ...
      resolve.
- [x] 2.10 Read the rewritten file end to end. Verify it still answers "where
      do I start" for a new contributor without opening a spec.

## 3. `cli/DEVELOPMENT.md` (~820 of 1,099 lines in scope)

- [x] 3.1 Audit all 22 sections into the three buckets; confirm § Working in
      this package, § Module map, § Packing, § Testing, § CI and releasing and
      § Known wrinkles are bucket 2. Verify every section is assigned.
- [x] 3.2 Generalize the existing preamble (lines 12–21) — which already names
...
- [x] 3.3 Convert § Authentication (99), § The command surface (73), § Reading
...
- [x] 3.4 Verify `cli/README.md` is untouched (`git diff --stat` shows no
      change to it) — it is out of scope per design.md § Non-Goals.

## 4. `ui/README.md` (~240 of 390 lines in scope)

- [x] 4.1 Convert § Admin (74), § Dashboard (42), § Settings (41) and
...
- [x] 4.2 Decide § Manual QA Matrix (73 lines): it is a test procedure, not a
...
- [x] 4.3 Verify § Stack, § Local Run, § App Structure and § Playwright
      Browser Testing are unchanged.

## 5. `tf/README.md`

- [x] 5.1 Convert § Build subsystem (14) and § Tenant database cluster (8) →
      `build-worker`, `build-execution`, `tenant-database-cluster`. Verify
      § Secrets (126 lines, operational) is unchanged.

## 6. Close-out

- [x] 6.1 Resolve design.md § Open Questions: confirm the root `README.md` and
      `k8s/README.md` need no entry, or convert them. Record the answer in the
      change before archiving.
- [x] 6.2 Verify `.github/workflows/ci.yml` is unchanged (`git diff --stat`
      shows no change to it) — this change adds no CI job.
- [x] 6.3 Confirm no bucket-3 content was lost: for each document, diff the
...
- [x] 6.4 Report the final line counts per document against the ~2,400-line
