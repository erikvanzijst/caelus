...
Provisioning therefore runs `REVOKE CONNECT ON DATABASE <db> FROM PUBLIC` on every
tenant database, and the same revocation is applied to the cluster's `postgres` and
...
--  5. REVOKE CONNECT ON DATABASE ... FROM PUBLIC   (always; D1)
...
evaluation. **Its softness is deliberate.** At 100% an honest tenant's ORM sees a
...
The hard block is three actions, because `NOLOGIN` alone stops nothing under
transaction pooling — the pooler's server connections are already authenticated and get
reused: set `NOLOGIN`, set the suspension flag so the D9 view stops resolving the role,
and terminate the role's backends.
...
### D9. Suspension is control-plane state, not a pooler command

The pooler's `auth_query` resolves against a platform-owned view that filters out
suspended deployments, so suspension is a row update: no pooler admin connection, no
per-instance fan-out, and it scales to any instance count. `NOLOGIN` remains the exact
backstop — one connection, no fan-out, undefeatable by the owner.

This matters because the pooler runs two instances behind a Service, and `RELOAD`,
`KILL` and `RECONNECT` are per-process with per-instance `auth_query` caches. Admin
commands are therefore an *accelerator* for immediacy, never load-bearing.

Two further consequences of running two instances, recorded because each is a trap
later: server connections multiply by instance count, so `default_pool_size ×
instances` is what lands on `max_connections`; and every client-side limit is
per-instance, so a limit of 25 permits 50. That second point is why there is one global
`max_user_connections` and no per-plan tiering — a per-plan number would not mean what
the plan says. **If exact connection enforcement is ever needed it must be
`ALTER ROLE ... CONNECTION LIMIT`**, which is instance-agnostic, at the cost of being in
backend rather than client units.

`auth_dbname` must be pinned to a platform-owned database. Without it the pooler runs
`auth_query` inside the client's *target* database, which the tenant owns and can
create objects in.
...
`DROP ROLE`, by then dependency-free.
...
CREATE TABLE deployment_database (
    ...
    suspended          BOOLEAN      NOT NULL DEFAULT FALSE,  -- read by the D9 view
...
2. **Infrastructure** — `tf/app` PostgreSQL 18 and PgBouncer ×2 per workspace with
   D13's configuration; bootstrap revocations, the `auth_query` view and its pinned
   `auth_dbname` database.
