...
- [x] 1.1 In `products/_lib/caelus-sftp/templates/_resources.tpl`, add
...
- [x] 1.2 In the same file, add a `startupProbe` and a `livenessProbe` to the
...
- [x] 1.3 Bump `products/_lib/caelus-sftp/Chart.yaml` `version` to `0.2.0` and update
...
- [x] 2.1 Add `api/tests/test_sftp_service_reachability.py` following the existing
...
- [x] 3.1 `helloworld`: `helm dependency build products/helloworld/chart`, remove the
      ...
      record the digest.
- [x] 3.2 `immich`: same sequence, `0.1.4` → `0.1.5`. Verify by pull; record the digest.
- [x] 3.3 `lemmy`: same sequence, `0.4.0` → `0.4.1`. Verify by pull; record the digest.
- [x] 3.4 `mattermost`: same sequence, `1.0.12` → `1.0.13`. Verify by pull; record the
      digest.
- [x] 3.5 `nextcloud`: same sequence, `0.1.9` → `0.1.10`. Verify by pull; record the
...
- [x] 3.6 `vaultwarden`: same sequence, `3.3.0` → `3.3.1`. Verify by pull; record the
      digest.

## 4. Repoint the recorded chart versions

- [x] 4.1 Update `chart_version` in `products/catalog/immich.yaml` (`0.1.5`),
...
- [x] 4.2 **Operator action, not a repository edit:** update the
...
- [x] 5.1 Deploy or redeploy one consuming product on dev at its new chart version and
...
- [x] 5.2 Break that deployment's application container deliberately (an image or
...
- [x] 5.3 Confirm the sidecar is not restarted while the application container is
...
- [x] 5.4 Confirm the startup probe does not kill the sidecar during host-key
...
- [x] 5.5 Restore the deployment to a healthy image and confirm SFTP still works,
      leaving no test wreckage on dev.

## 6. Documentation

- [x] 6.1 Update `products/_lib/caelus-sftp/README.md` if 1.3 left anything unstated —
...
- [x] 6.2 Add a release/changelog note framing this as a **fix**: tenants whose
      ...
      **Not landed in the repo:** the note was written, but this repository keeps no
      changelog and adding one was declined — a convention nobody maintains is worse
      than none. The user-visible framing belongs wherever release notes are actually
      published; nothing in the specs depends on a file here.
- [x] 6.3 Add a pointer in `var/ssh_access.md` D17 recording that the SFTP half shipped
