...
- [ ] 1.1 Take `FREEPOD_SESSION_ROOT` as a required input in `entrypoint.sh`, accepting
...
- [ ] 1.2 Resolve the loader and library paths for the sidecar's own `sftp-server` at build
      time and record them in the image, rather than hardcoding a path in `dispatch.sh`.
      Verify the recorded paths exist in the built image.
- [ ] 1.3 Serve every file-transfer request from the sidecar's own `sftp-server`, chrooted
      into the session root, via the loader indirection (design.md § D3). Remove the search
      of `$app_root` for a helper. Verify a transfer succeeds against a target image
      containing no `sftp-server`, `scp` or `rsync`.
- [ ] 1.4 Derive read-only from the session root's filesystem (`statvfs` → `ST_RDONLY` →
      `sftp-server -R`) rather than from a flag. Verify a read-only mount declines writes and
      a writable root accepts them, with no configuration distinguishing the two.
- [ ] 1.5 Route on the declared session root before anything else: under `volume:`, refuse a
...
- [ ] 1.6 Split the database-tooling gate in two: the session root decides whether the
...
- [ ] 1.7 Refuse to serve from the sidecar's own filesystem when an `app-container` root
      cannot be resolved. Verify the message names the likely cause and that no session opens.
- [ ] 1.8 Extend `test/run-tests.sh` to run the whole matrix: both session roots × (shell,
...
- [ ] 1.9 Update `products/_lib/ssh-sidecar-image/README.md` for the new input and the
...
- [ ] 2.1 Replace `ssh-sidecar.sftp.*` and `ssh-sidecar.dev.*` with one helper set taking
      `sessionRoot`, failing the render when it is absent or malformed. Verify a chart
      calling it without one does not render.
- [ ] 2.2 Render the platform sidecar image, pinned from a system value, for every session
      root. Verify a tenant value cannot change the image reference.
- [ ] 2.3 Accept mounts only for a `volume:` session root, rendering each `readOnly: true`,
      and refuse mounts alongside `app-container`. Verify both.
- [ ] 2.4 Stop rendering the credentials Secret and the sshd-init ConfigMap; pass the trusted
...
- [ ] 2.5 Keep the shared Service helper unchanged — name, port, `publishNotReadyAddresses`,
...
- [ ] 2.6 Bump the library chart version and update `products/_lib/ssh-sidecar-chart`'s
      README for the single helper set. Verify `helm lint` passes.

## 3. Products

- [ ] 3.1 Move `helloworld`, `immich`, `lemmy`, `mattermost`, `nextcloud` and `vaultwarden`
...
- [ ] 3.2 Move `products/custom` to `sessionRoot: app-container`, keeping
...
- [ ] 3.3 Vendor the new library version into every consumer and update each `Chart.lock`.
      Verify `helm template` succeeds for all seven charts.
- [ ] 3.4 Replace the hand-listed `CONSUMERS` map in `api/tests/test_sftp_service_reachability.py`
...
- [ ] 3.5 Assert that no chart renders `atmoz/sftp`, no chart renders an SSH credentials
