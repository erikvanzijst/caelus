...
## Purpose
Inject system-controlled per-app TLS settings (`caelus.tls`) into Helm values and wire product chart Ingresses (websecure-only; cert-manager + `tls:` for custom domains).
