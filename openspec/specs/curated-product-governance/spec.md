# curated-product-governance Specification

## Purpose

Defines the columns that make a product catalog-managed — `slug`, `curated`,
and `catalog_commit` — the service-layer write guards that keep curated
products read-only across REST, CLI, and the admin UI, the break-glass
override for modifications, and the rule that only the reconciler writes
curation state.
