...
Users interact with their deployments through the web UI, so file-access credentials must be surfaced there. This capability adds a dedicated SFTP panel to the deployment detail view that displays the host, port, username, and password (masked by default, revealable, with copy affordances), and communicates that access is read-only. The panel appears only for deployments that actually have file access, driven by the credentials API's absence semantics, and is implemented as its own reusable component rather than inlined into a page-level component.
